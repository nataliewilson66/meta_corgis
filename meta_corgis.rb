...
# SOLUTION USING method_missing:

  # def method_missing(name, *args)
  #   # Your code goes here...
  #   method_name = name.to_s
  #   info_str = "get_#{method_name}_info"
  #   tastiness_str = "get_#{method_name}_tastiness"
  #   info = @snack_box.send(info_str, @box_id)
  #   tastiness = @snack_box.send(tastiness_str, @box_id)
  #   result = "#{method_name.capitalize}: #{info}: #{tastiness} "
  #   tastiness > 30 ? "* #{result}" : result
  # end


# SOLUTION USING define_method:

  def self.define_snack(name)
    # Your code goes here...
    define_method(name) {
      info_str = "get_#{name}_info"
      tastiness_str = "get_#{name}_tastiness"
      info =  @snack_box.send(info_str, @box_id)
      tastiness = @snack_box.send(tastiness_str, @box_id)
